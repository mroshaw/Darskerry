...
namespace DaftAppleGames.Editor.Lighting
{
    ...
    public class LightProbeVolume : MonoBehaviour
    {
        ...
        [SerializeField] private List<LightProbeIncludeVolume> m_includeVolumes = new List<LightProbeIncludeVolume>();
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (m_drawProbePositionGizmos)
            {
                ...
                {
                    foreach (LightProbeIncludeVolume ignoreVolume in m_includeVolumes)
                    {
        ...
        public void AddIncludeVolume(LightProbeIncludeVolume volume)
        {
        ...
        /// </summary>
        /// <param name="volume"></param>
        public void RemoveIncludeVolume(LightProbeIncludeVolume volume)
        {
        ...
        {
            LightProbeIncludeVolume[] volumes = GameObject.FindObjectsOfType<LightProbeIncludeVolume>();
            foreach (LightProbeIncludeVolume volume in volumes)
            {
                if (volume == null)
                {
                    continue;
            ...
            for (int x = 0; x < m_densityX; x++)
            {
                for (int y = 0; y < m_densityY; y++)
                {
                    for (int z = 0; z < m_densityZ; z++)
                    {
                        ...
                        {
                            foreach (LightProbeIncludeVolume volume in m_includeVolumes)
                            {
                                if (volume == null)
                                {
                                    continue;

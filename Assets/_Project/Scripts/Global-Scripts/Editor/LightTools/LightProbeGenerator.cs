...
namespace DaftAppleGames.Editor.Lighting
{
    ...
    public class LightProbeGenerator
    {
        ...
        private static LightProbeGeneratorSettings m_settings;
        ...
        {
            LightProbeIncludeVolume[] probeVolumes = GameObject.FindObjectsOfType<LightProbeIncludeVolume>();
            if (probeVolumes.Length > 0)
            {
                foreach (LightProbeIncludeVolume volume in probeVolumes)
                {
